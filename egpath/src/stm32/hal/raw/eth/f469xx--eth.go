...
const (
	...
	TFRS                MACDBGR = 0x03 << 20 //+ Tx FIFO read status mask.
	TFRS_WRITING        MACDBGR = 0x03 << 20 //  Writing the received TxStatus or flushing the TxFIFO.
	TFRS_WAITING        MACDBGR = 0x02 << 20 //  Waiting for TxStatus from MAC transmitter.
	TFRS_READ           MACDBGR = 0x01 << 20 //  Read state (transferring data to the MAC transmitter).
	TFRS_IDLE           MACDBGR = 0x00 << 20 //  Idle state.
	MTP                 MACDBGR = 0x01 << 19 //+ MAC transmitter in pause.
	MTFCS               MACDBGR = 0x03 << 17 //+ MAC transmit frame controller status mask.
	MTFCS_TRANSFERRING  MACDBGR = 0x03 << 17 //  Transferring input frame for transmission.
	MTFCS_GENERATINGPCF MACDBGR = 0x02 << 17 //  Generating and transmitting a Pause control frame (in full duplex mode).
	MTFCS_WAITING       MACDBGR = 0x01 << 17 //  Waiting for Status of previous frame or IFG/backoff period to be over.
	MTFCS_IDLE          MACDBGR = 0x00 << 17 //  Idle.
	MMTEA               MACDBGR = 0x01 << 16 //+ MAC MII transmit engine active.
	RFFL                MACDBGR = 0x03 << 8  //+ Rx FIFO fill level mask.
	RFFL_FULL           MACDBGR = 0x03 << 8  //  RxFIFO full.
	RFFL_ABOVEFCT       MACDBGR = 0x02 << 8  //  RxFIFO fill-level above flow-control activate threshold.
	RFFL_BELOWFCT       MACDBGR = 0x01 << 8  //  RxFIFO fill-level below flow-control de-activate threshold.
	RFFL_EMPTY          MACDBGR = 0x00 << 8  //  RxFIFO empty.
	RFRCS               MACDBGR = 0x03 << 5  //+ Rx FIFO read controller status mask.
	RFRCS_FLUSHING      MACDBGR = 0x03 << 5  //  Flushing the frame data and status.
	RFRCS_STATUSREADING MACDBGR = 0x02 << 5  //  Reading frame status (or time-stamp).
	RFRCS_DATAREADING   MACDBGR = 0x01 << 5  //  Reading frame data.
	RFRCS_IDLE          MACDBGR = 0x00 << 5  //  IDLE state.
	RFWRA               MACDBGR = 0x01 << 4  //+ Rx FIFO write controller active.
	MSFRWCS             MACDBGR = 0x03 << 1  //+ MAC small FIFO read / write controllers status  mask.
